...
module API
  class GoalsAPI < Grape::API
    ...
    resource :goals do
      ...
      get :nearby do
        @goals = Goal.select("DISTINCT ON (user_id) *, st_distance(location, 'point(#{params[:longitude]} #{params[:latitude]})') as distance").order("user_id, distance ASC, goals.id DESC")
